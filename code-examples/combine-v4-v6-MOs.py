...
Look for IPv6 matches, determine that no functionality is lost, and merge it
with the IPv4 version, or just a report that makes suggesions for IPv6 MOs to
merge into IPv4 MOs, perhaps based on the name of the MO (e.g., if AT&T_v6 and
AT&T both exist, can they safely be merged? What would be new MO look like?
Perhaps produce the JSON for a PATCH to AT&T to move the IPv6 stuff into it.)
Or, if you don't like the idea of using names, take a list of MOs that looks
like:
